...
DEFAULT_GITHUB_REPO="$(detect_origin_repo)"
if [ -z "$DEFAULT_GITHUB_REPO" ]; then
    DEFAULT_GITHUB_REPO="p6118810-hash/codeorb"
fi

GITHUB_REPO="${CODEORB_GITHUB_REPO:-$DEFAULT_GITHUB_REPO}"

# Website repo for auto-updating appcast
WEBSITE_DIR="${CODEORB_WEBSITE_DIR:-$PROJECT_DIR/../CodeOrb-website}"
WEBSITE_PUBLIC="$WEBSITE_DIR/public"

APP_PATH="$EXPORT_PATH/CodeOrb.app"
APP_NAME="CodeOrb"
...
POSSIBLE_PATHS=(
    "$HOME/Library/Developer/Xcode/DerivedData/CodeOrb-*/SourcePackages/artifacts/sparkle/Sparkle/bin"
)
...
if ! command -v gh &> /dev/null; then
    ...
    if gh release view "v$VERSION" --repo "$GITHUB_REPO" &>/dev/null; then
        ...
        gh release create "v$VERSION" "$DMG_PATH" \
            --repo "$GITHUB_REPO" \
            --title "CodeOrb v$VERSION" \
            --notes "## CodeOrb v$VERSION

### Installation
1. Download \`$APP_NAME-$VERSION.dmg\`
2. Open the DMG and drag CodeOrb to Applications
3. Launch CodeOrb from Applications

### Auto-updates
After installation, CodeOrb will automatically check for updates."
    fi
...
EOF
    ...
    WRANGLER_PROJECT="${CODEORB_WRANGLER_PROJECT:-codeorb-website}"
