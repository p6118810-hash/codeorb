...
struct TerminalVisibilityDetector {
    ...
    /// Check if a monitored session is currently focused (user is looking at it)
    /// - Parameter sessionPid: The PID of the Codex session process
    ...
    static func isSessionFocused(sessionPid: Int) async -> Bool {
        ...
        if isInTmux {
            ...
            return await TmuxTargetFinder.shared.isSessionPaneActive(sessionPid: sessionPid)
        } else {
