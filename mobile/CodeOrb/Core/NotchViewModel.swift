...
class NotchViewModel: ObservableObject {
    ...
    @Published var compactWindowSize: CGSize = CGSize(width: 76, height: 140)

    // MARK: - Dependencies

    private let screenSelector = ScreenSelector.shared
    private let soundSelector = SoundSelector.shared
    private let codexDirSelector = CodexDirSelector.shared
    ...
    private let defaultPanelWidthLimit: CGFloat = 480
    private let defaultPanelHeight: CGFloat = 540
    ...
    var openedSize: CGSize {
        ...
        case .menu:
            // Keep the settings panel and instances panel aligned on first open,
            // then add picker expansion deltas only when settings rows expand.
            return CGSize(
                width: min(screenRect.width * 0.4, defaultPanelWidthLimit),
                height: defaultPanelHeight
                    + screenSelector.expandedPickerHeight
                    + soundSelector.expandedPickerHeight
                    + codexDirSelector.expandedPickerHeight
            )
        case .instances:
            return CGSize(
                width: min(screenRect.width * 0.4, defaultPanelWidthLimit),
    ...
    private func observeSelectors() {
        ...
        codexDirSelector.$isPickerExpanded
            .sink { [weak self] _ in self?.objectWillChange.send() }
            .store(in: &cancellables)
    }
    ...
    func setCompactWindowSize(_ size: CGSize) {
        guard compactWindowSize != size else { return }
        compactWindowSize = size
    }
