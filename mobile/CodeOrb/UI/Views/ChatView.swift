...
struct ChatView: View {
    ...
    @State private var isLocatorAvailable: Bool = false
    ...
    var body: some View {
        ...
        .task(id: locatorAvailabilityKey) {
            isLocatorAvailable = await TerminalLocator.shared.canLocate(session: session)
        }
    ...
    private var interactivePromptBar: some View {
        ChatInteractivePromptBar(
            isLocatorAvailable: isLocatorAvailable,
    ...
    private func focusTerminal() {
        Task {
            _ = await TerminalLocator.shared.focus(session: session)
        }
    }
...
struct ProcessingIndicatorView: View {
    ...
    private let color = Color(red: 0.85, green: 0.47, blue: 0.34) // warm activity orange
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                .foregroundColor(isLocatorAvailable ? .black : .white.opacity(0.4))
                .padding(.horizontal, 16)
                .padding(.vertical, 8)
                .background(isLocatorAvailable ? Color.white.opacity(0.95) : Color.white.opacity(0.1))
                .clipShape(Capsule())
            }
            .buttonStyle(.plain)
...
struct NewMessagesIndicator: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .background(
                Capsule()
                    .fill(Color(red: 0.85, green: 0.47, blue: 0.34)) // warm activity orange
