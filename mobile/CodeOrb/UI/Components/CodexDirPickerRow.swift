//
//  CodexDirPickerRow.swift
//  CodeOrb
//
//  Settings row for choosing Codex's config directory. Expands inline
...
struct CodexDirPickerRow: View {
    @ObservedObject private var selector = CodexDirSelector.shared
    @State private var currentValue: String = AppSettings.codexDirectoryPath
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Button {
                ...
                HStack(spacing: 10) {
                    ...
                    Text(AppText.get(.menuCodexDirectory))
            ...
            if isExpanded {
                VStack(spacing: 2) {
                    CodexDirOptionRow(
                        label: AppText.get(.menuCodexDirectoryAutoDetect),
                    ...
                    CodexDirOptionRow(
                        label: AppText.get(.menuCodexDirectoryChooseFolder),
        ...
        .onAppear { currentValue = AppSettings.codexDirectoryPath }
    }
    ...
    private func openFolderPicker() {
        ...
        panel.title = AppText.get(.menuCodexDirectoryDialogTitle)
        panel.message = AppText.get(.menuCodexDirectoryDialogMessage)
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.allowsMultipleSelection = false
        panel.showsHiddenFiles = true
        panel.canCreateDirectories = false
        panel.directoryURL = CodexPaths.codexDir
    ...
    private func applyChoice(path: String) {
        currentValue = path
        AppSettings.codexDirectoryPath = path
        CodexPaths.invalidateCache()
...
private struct CodexDirOptionRow: View {
    let label: String
    let sublabel: String?
    let isSelected: Bool
    let action: () -> Void
