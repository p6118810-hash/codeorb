...
struct ScreenPickerRow: View {
        ...
        VStack(spacing: 0) {
            ...
            if isExpanded {
                VStack(spacing: 2) {
                    // Automatic option
                    ScreenOptionRow(
                        label: AppText.get(.menuScreenAutomatic),
                        sublabel: AppText.get(.menuScreenAutomaticSublabel),
    ...
    private var currentSelectionLabel: String {
        ...
        case .automatic:
            return AppText.get(.menuScreenAuto)
        case .specificScreen:
    ...
    private func screenSublabel(for screen: NSScreen) -> String? {
        ...
        if screen.isBuiltinDisplay {
            parts.append(AppText.get(.menuScreenBuiltin))
        }
        if screen == NSScreen.main {
            parts.append(AppText.get(.menuScreenMain))
        }
