...
private let logger = Logger(subsystem: "com.codeorb", category: "Hooks")

/// Event received from Codex hooks
struct HookEvent: Codable, Sendable {
    ...
    init(provider: String?, sessionId: String, cwd: String, event: String, status: String, pid: Int?, tty: String?, tool: String?, toolInput: [String: AnyCodable]?, toolUseId: String?, notificationType: String?, message: String?, transcriptPath: String?, prompt: String?, lastAssistantMessage: String?) {
    ...
    var sessionProvider: SessionProviderKind {
        guard let provider,
              let parsed = SessionProviderKind(rawValue: provider.lowercased()) else {
            return transcriptPath == nil ? .claude : .codex
        }
        return parsed
    }
...
/// Unix domain socket server that receives events from Codex hooks
/// Uses GCD DispatchSource for non-blocking I/O
class HookSocketServer {
    static let shared = HookSocketServer()
    static let socketPath = "/tmp/codeorb.sock"
    static let legacySocketPath = "/tmp/claude-island.sock"

    private struct SocketListener {
        let path: String
        let defaultProvider: SessionProviderKind
    }

    private static let listeners: [SocketListener] = [
        SocketListener(path: socketPath, defaultProvider: .codex),
        SocketListener(path: legacySocketPath, defaultProvider: .claude),
    ]

    private var serverSockets: [String: Int32] = [:]
    private var acceptSources: [String: DispatchSourceRead] = [:]
    private var eventHandler: HookEventHandler?
    private var permissionFailureHandler: PermissionFailureHandler?
    private let queue = DispatchQueue(label: "com.codeorb.socket", qos: .userInitiated)
    ...
    private func startServer(onEvent: @escaping HookEventHandler, onPermissionFailure: PermissionFailureHandler?) {
        guard serverSockets.isEmpty else { return }

        eventHandler = onEvent
        permissionFailureHandler = onPermissionFailure

        for listener in Self.listeners {
            guard let socket = makeListener(for: listener) else { continue }
            serverSockets[listener.path] = socket

            let acceptSource = DispatchSource.makeReadSource(fileDescriptor: socket, queue: queue)
            acceptSource.setEventHandler { [weak self] in
                self?.acceptConnection(serverSocket: socket, defaultProvider: listener.defaultProvider)
            }
            acceptSource.setCancelHandler { [weak self] in
                close(socket)
                self?.serverSockets.removeValue(forKey: listener.path)
            }
            acceptSource.resume()
            acceptSources[listener.path] = acceptSource

            logger.info("Listening on \(listener.path, privacy: .public)")
        }
    }

    private func makeListener(for listener: SocketListener) -> Int32? {
        unlink(listener.path)

        let serverSocket = socket(AF_UNIX, SOCK_STREAM, 0)
        guard serverSocket >= 0 else {
            logger.error("Failed to create socket for \(listener.path, privacy: .public): \(errno)")
            return nil
        }
        ...
        listener.path.withCString { ptr in
        ...
        guard bindResult == 0 else {
            logger.error("Failed to bind \(listener.path, privacy: .public): \(errno)")
            close(serverSocket)
            return nil
        }

        chmod(listener.path, 0o600)

        guard listen(serverSocket, 10) == 0 else {
            logger.error("Failed to listen on \(listener.path, privacy: .public): \(errno)")
            close(serverSocket)
            return nil
        }
    ...
    /// Cancel all pending permissions for a session when Codex stops waiting.
    ...
    private func acceptConnection(serverSocket: Int32, defaultProvider: SessionProviderKind) {
        ...
        handleClient(clientSocket, defaultProvider: defaultProvider)
    }

    private func handleClient(_ clientSocket: Int32, defaultProvider: SessionProviderKind) {
        ...
        guard let rawEvent = try? JSONDecoder().decode(HookEvent.self, from: data) else {
        ...
        let event = HookEvent(
            provider: rawEvent.provider ?? defaultProvider.rawValue,
            ...
            notificationType: rawEvent.notificationType,
            message: rawEvent.message,
            transcriptPath: rawEvent.transcriptPath,
            prompt: rawEvent.prompt,
            lastAssistantMessage: rawEvent.lastAssistantMessage
        )
