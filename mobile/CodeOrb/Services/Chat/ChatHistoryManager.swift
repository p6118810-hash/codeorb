...
struct ToolCallItem: Equatable, Sendable {
    ...
    /// legacy name; the current CLI now uses "Agent".
    ...
    var inputPreview: String {
        ...
        if let step = input["step"] {
            let status = input["status"].map { $0.replacingOccurrences(of: "_", with: " ") }
            return status.map { "\(step) · \($0)" } ?? step
        }
        if let description = input["description"] {
            return MCPToolFormatter.summarizeDisplayText(description, maxLength: 60) ?? description
        }
        if let prompt = input["prompt"] {
            return MCPToolFormatter.summarizeDisplayText(prompt, maxLength: 60) ?? prompt
        }
        if let question = input["question"] {
            return MCPToolFormatter.summarizeDisplayText(question, maxLength: 60) ?? question
        }
        ...
        if let firstValue = input.values.first {
            return MCPToolFormatter.summarizeDisplayText(firstValue, maxLength: 60)
                ?? String(firstValue.prefix(60))
        }
        return ""
    }
...
struct SubagentToolCall: Equatable, Identifiable, Sendable {
    ...
    var displayText: String {
        ...
        case "Edit":
            if let path = input["file_path"] ?? input["path"] {
                return "修改 \(URL(fileURLWithPath: path).lastPathComponent)"
            }
            return "修改中..."
        case "Write":
            if let path = input["file_path"] ?? input["path"] {
