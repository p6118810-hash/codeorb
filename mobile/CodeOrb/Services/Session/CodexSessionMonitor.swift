...
class CodexSessionMonitor: ObservableObject {
    ...
    func startMonitoring() {
        Task {
            await SessionStore.shared.bootstrapSessions()
        }
        ...
        HookSocketServer.shared.start(
            onEvent: { event in
                ...
                if event.sessionPhase == .processing && event.transcriptPath == nil {
                    Task { @MainActor in
    ...
    private func updateFromSessions(_ sessions: [SessionState]) {
        instances = sessions
        // Only approval requests should trigger the "pending" notification flow.
