...
//  CodeOrb
//
//  Parses Codex JSONL conversation files to extract summary and last message
...
actor ConversationParser {
    ...
    nonisolated static let logger = Logger(subsystem: "com.codeorb", category: "Parser")
    ...
    private static func formatToolInput(_ input: [String: Any]?, toolName: String) -> String {
        ...
        case "Task", "Agent":
            // "Task" is the legacy name; the current CLI now uses "Agent".
    ...
    func parseFullConversation(
        sessionId: String,
        cwd: String,
        provider: SessionProviderKind = .codex
    ) -> [ChatMessage] {
        let sessionFile = Self.sessionFilePath(sessionId: sessionId, cwd: cwd, provider: provider)
        let stateKey = Self.stateKey(sessionId: sessionId, provider: provider)
    ...
    func completedToolIds(
        for sessionId: String,
        provider: SessionProviderKind = .codex
    ) -> Set<String> {
        return incrementalState[Self.stateKey(sessionId: sessionId, provider: provider)]?.completedToolIds ?? []
    }
    ...
    func toolResults(
        ...
        return incrementalState[Self.stateKey(sessionId: sessionId, provider: provider)]?.toolResults ?? [:]
    }
    ...
    func structuredResults(
        ...
        return incrementalState[Self.stateKey(sessionId: sessionId, provider: provider)]?.structuredResults ?? [:]
    }

    /// Reset incremental state for a session (call when reloading)
    func resetState(for sessionId: String, provider: SessionProviderKind = .codex) {
        incrementalState.removeValue(forKey: Self.stateKey(sessionId: sessionId, provider: provider))
    }

    /// Check if a /clear command was detected during the last parse
    /// Returns true once and consumes the pending flag
    func checkAndConsumeClearDetected(
        for sessionId: String,
        provider: SessionProviderKind = .codex
    ) -> Bool {
        let stateKey = Self.stateKey(sessionId: sessionId, provider: provider)
    ...
    private static func sessionFilePath(
        sessionId: String,
        cwd: String,
        provider: SessionProviderKind
    ) -> String {
        let projectDir = cwd.replacingOccurrences(of: "/", with: "-").replacingOccurrences(of: ".", with: "-")
        return transcriptProjectsRoot(for: provider).path + "/" + projectDir + "/" + sessionId + ".jsonl"
    }

    private static func transcriptProjectsRoot(for provider: SessionProviderKind) -> URL {
        switch provider {
        case .codex:
            return CodexPaths.projectsDir
        case .claude:
    ...
    private static func stateKey(sessionId: String, provider: SessionProviderKind) -> String {
        "\(provider.rawValue):\(sessionId)"
    }

    /// Build subagent JSONL file path.
    ///
    /// Current Codex builds nest subagent files under the parent session:
    ///   projects/<project>/<sessionId>/subagents/agent-<agentId>.jsonl
    ///
    /// Older builds stored them flat:
    ...
    nonisolated static func subagentFilePath(sessionId: String, agentId: String, projectDir: String) -> String {
        let base = CodexPaths.projectsDir.path + "/" + projectDir
        ...
        if let content = messageDict["content"] as? String {
            ...
            for block in contentArray {
                if let blockType = block["type"] as? String {
                    ...
                    case "image":
                        // Codex stores inline images as base64 with media_type.
