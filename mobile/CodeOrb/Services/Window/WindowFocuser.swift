...
actor WindowFocuser {
    ...
    func focusTmuxWindow(terminalPid: Int, windows: [YabaiWindow]) async -> Bool {
        ...
        // Fall back to the main terminal window when yabai does not expose a tmux-specific title.
        if let window = WindowFinder.shared.findPrimaryWindow(forTerminalPid: terminalPid, windows: windows) {
