...
actor YabaiController {
    ...
    /// Focus the terminal window for a given session PID (tmux only)
    func focusWindow(forSessionPid sessionPid: Int) async -> Bool {
        ...
        return await focusTmuxInstance(sessionPid: sessionPid, tree: tree, windows: windows)
    }
    ...
    private func focusTmuxInstance(sessionPid: Int, tree: [Int: ProcessInfo], windows: [YabaiWindow]) async -> Bool {
        // Find the tmux target for this session process.
        guard let target = await TmuxController.shared.findTmuxTarget(forSessionPid: sessionPid) else {
            return false
        }
    ...
    private func focusTmuxPane(forWorkingDir workingDir: String, tree: [Int: ProcessInfo], windows: [YabaiWindow]) async -> Bool {
        ...
        do {
            ...
            for pane in panes {
                ...
                // Check if this pane has a managed CLI child with matching cwd.
                for (pid, info) in tree {
                    let isChild = ProcessTreeBuilder.shared.isDescendant(targetPid: pid, ofAncestor: panePid, tree: tree)
                    let isManagedCLI = isManagedCLIProcess(info.command)

                    guard isChild, isManagedCLI else { continue }
    ...
    private nonisolated func isManagedCLIProcess(_ command: String) -> Bool {
        let normalized = command.lowercased()
        // Keep legacy Claude matching so older panes still focus correctly after
        // the CodeOrb rename and CLI migration.
