...
//  CodeOrb
//
//  Structured models for all Codex tool results
//
...
struct ToolStatusDisplay {
    ...
    static func running(for toolName: String, input: [String: String]) -> ToolStatusDisplay {
        ...
        case "Read":
            if let path = input["file_path"] ?? input["path"] {
                return ToolStatusDisplay(text: "读取 \(URL(fileURLWithPath: path).lastPathComponent)", isRunning: true)
            }
            return ToolStatusDisplay(text: "读取中...", isRunning: true)
        case "Edit":
            if let path = input["file_path"] ?? input["path"] {
                return ToolStatusDisplay(text: "修改 \(URL(fileURLWithPath: path).lastPathComponent)", isRunning: true)
            }
            return ToolStatusDisplay(text: "修改中...", isRunning: true)
        case "Write":
            if let path = input["file_path"] ?? input["path"] {
                return ToolStatusDisplay(text: "写入 \(URL(fileURLWithPath: path).lastPathComponent)", isRunning: true)
            }
    ...
    static func completed(for toolName: String, result: ToolResultData?) -> ToolStatusDisplay {
        ...
        case .edit(let r):
            return ToolStatusDisplay(text: "修改 \(r.filename)", isRunning: false)

        case .write(let r):
            let action = r.type == .create ? "新建" : "写入"
