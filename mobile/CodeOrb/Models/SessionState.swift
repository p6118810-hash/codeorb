...
//  CodeOrb
//
//  Unified state model for a Codex session.
//  Consolidates all state that was previously spread across multiple components.
//

import Foundation

enum SessionProviderKind: String, CaseIterable, Equatable, Sendable {
    case codex
    case claude
    case gemini
...
/// Complete state for a single Codex session
...
struct SessionState: Equatable, Identifiable, Sendable {
    ...
    var isCodexSession: Bool {
        provider == .codex && transcriptPath != nil
    }
    ...
    var supportsChatHistory: Bool {
        provider.supportsChatHistory && transcriptPath != nil
    }
}
